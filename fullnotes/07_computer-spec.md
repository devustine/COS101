...
This is what the GPU looks like. You can see it has fans to dissipate the heat generated by the device. The GPU is installed on the motherboard, along with other essential components of the computer. As mentioned earlier, components like the CPU, hard disk, and power supply are also mounted on the motherboard. The GPU sits in a dedicated slot on the motherboard, working alongside these components.

## RAM (Random Access Memory)

For a computer system, the next component we're gonna be looking at is RAM.

### Why RAM is Important
RAM is essential for running multiple applications simultaneously. For example, you might want to:
- Watch films
- Do some editing
- Transfer files to a particular location
- Download files

The ability to perform all these tasks depends on your RAM.
...
Different types of applications rely on your system’s RAM, including:
- Editing
- Playing games
- Doing some file transfer
- Doing some file download

All of these tasks depend heavily on RAM to run smoothly and efficiently.
